...
dxy = dxy.drop(['updateTime','province_suspectedCount','city_suspectedCount'], axis=1)
...
# Load the ccdc data to compare
with open('ccdc_t.txt','rb') as f:
    ccdc_t = pickle.load(f)

with open('ccdc_n.txt','rb') as f:
    ccdc_n = pickle.load(f)

# plotting
np = []
for p in period:
    np.append(p.strftime('%m%d'))
ccdc_n_tr=ccdc_n[dxy_p_n.columns]
ccdc_t_tr=ccdc_t[dxy_p_t.columns]

plt.plot(np, ccdc_t_tr.loc[('湖北省','confirmed_t')])
plt.plot(np, dxy_p_t.loc[('湖北省','confirmed_t')])